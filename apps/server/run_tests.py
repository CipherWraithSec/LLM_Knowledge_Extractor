...
async def test_analyze_endpoint():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as client:
        ...
        required_fields = ["id", "title", "topics", "sentiment", "keywords",
                           "summary", "original_text", "confidence_score", "createdAt"]
        ...
        assert isinstance(data["confidence_score"],
                          (int, float)) or data["confidence_score"] is None
...
async def test_search_endpoint(created_analysis):
    ...
    async with AsyncClient(base_url="http://localhost:8000") as client:
        ...
        if created_analysis["keywords"]:
            ...
            print(
                f"✅ Search (with topic '{search_term}') passed - found {len(search_results)} results")
